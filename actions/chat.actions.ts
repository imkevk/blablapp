...
import { ChatActionTypes, JOIN_FAILURE, JOIN_REQUEST, JOIN_SUCCESS, SEND_MESSAGE, SEND_MESSAGE_FAILURE, SEND_MESSAGE_SUCCESS, SYNC_FAILURE, SYNC_SUCCESS } from './types';
...
export const join = (user: string, room: string): ChatThunkActionType => {
  return dispatch => {
    const syncSuccess = (messages: Message[]) => dispatch({ type: SYNC_SUCCESS, messages });
    const syncFailure = (error: Error) => dispatch({ type: SYNC_FAILURE, error });

    dispatch({ type: JOIN_REQUEST, user, room });

    chatService.join(user, room, syncSuccess, syncFailure)
